...
static int hf_zbee_zcl_thermostat_cmd_id = -1;
static int hf_zbee_zcl_ias_zone_client_cmd_id = -1;
static int hf_zbee_zcl_ias_zone_client_zone_id = -1;
static int hf_zbee_zcl_ias_zone_client_erc = -1;
static int hf_zbee_zcl_ias_zone_cmd_id = -1;
static int hf_zbee_zcl_poll_control_client_cmd_id = -1;
static int hf_zbee_zcl_poll_control_server_cmd_id = -1;
static int hf_zbee_zcl_poll_control_client_sfp = -1;
static int hf_zbee_zcl_poll_control_client_fpt = -1;
static int hf_zbee_zcl_poll_control_client_nlpi = -1;
static int hf_zbee_zcl_poll_control_client_nspi = -1;
static int hf_zbee_zcl_identify_client_identify_time = -1;
static int hf_zbee_zcl_identify_server_timeout = -1;
static int hf_zbee_zcl_ias_zone_scn_alarm1 = -1;
...
static int hf_zbee_zcl_ias_zone_scn_alarm2 = -1;
static int hf_zbee_zcl_ias_zone_scn_tamper = -1;
static int hf_zbee_zcl_ias_zone_scn_battery = -1;
static int hf_zbee_zcl_ias_zone_scn_supervision_reports = -1;
static int hf_zbee_zcl_ias_zone_scn_restore_reports = -1;
static int hf_zbee_zcl_ias_zone_scn_trouble = -1;
static int hf_zbee_zcl_ias_zone_scn_ac_mains = -1;
static int hf_zbee_zcl_ias_zone_scn_ext_status = -1;
static int hf_zbee_zcl_thermostat_setpointrl_modes = -1;
static int hf_zbee_zcl_thermostat_sws_n_trans = -1;
static int hf_zbee_zcl_thermostat_sws_day_of_week = -1;
static int hf_zbee_zcl_thermostat_sws_mfs_heat = -1;
static int hf_zbee_zcl_thermostat_sws_mfs_cool = -1;
static int hf_zbee_zcl_thermostat_gws_mtr_heat = -1;
static int hf_zbee_zcl_thermostat_gws_mtr_cool = -1;
static int hf_zbee_zcl_thermostat_sws_trans_time_1 = -1;
static int hf_zbee_zcl_thermostat_sws_hsp_1 = -1;
static int hf_zbee_zcl_thermostat_sws_csp_1 = -1;
static int hf_zbee_zcl_thermostat_sws_trans_time_2 = -1;
static int hf_zbee_zcl_thermostat_sws_hsp_2 = -1;
static int hf_zbee_zcl_thermostat_sws_csp_2 = -1;
static int hf_zbee_zcl_thermostat_gws_days_to_return = -1;
...
static gint ett_zbee_zcl_sws_dow_for_sequence = -1;
static gint ett_zbee_zcl_gws_days_to_return = -1;
static gint ett_zbee_zcl_gws_mtr = -1;
static gint ett_zbee_zcl_scn_zone_status = -1;
static gint ett_zbee_zcl_qnir_field_control = -1;
static gint ett_zbee_zcl_ibr_field_control = -1;
static gint ett_zbee_zcl_sws_mfs = -1;
...
static const value_string zbee_zcl_cmd_names[] = {
    ...
    { ZBEE_ZCL_CMD_DISCOVER_COMMANDS_RECEIVED, "Discover Commands Received" },
    { ZBEE_ZCL_CMD_DISCOVER_COMMANDS_RECEIVED_RESPONSE, "Discover Commands Received Response" },
...
/* OTA Upgrade Server Image Notify Payload Type */
...
/* OTA Upgrade Server Query Next Image Response Status */
...
/* Thermostat Cluster Client Commands */
static const value_string zbee_zcl_thermostat_cmd_names[] = {
...
/* IAS Zone Cluster Client Commands */
...
/* IAS Zone Cluster Server Commands */
static const value_string zbee_zcl_ias_zone_cmd_names[] = {
...
/* IAS Zone Enroll Response Code Commands */
...
/* Poll Control Cluster Client Commands */
...
/* Identify Cluster Client Commands */
...
/* OTA Upgrade Cluster Client Commands */
...
/* OTA Upgrade Cluster Server Commands */
...
/* Identify Cluster Server Commands */
...
/* Poll Control Cluster Server Commands */
...
/* Thermostat Cluster Setpoint Raise/Lower Mode Fields */
static const value_string zbee_zcl_thermostat_setpointrl_modes[] = {
...
/* Thermostat Cluster Set Weekly Schedule Day of Week for Sequence */
static const value_string zbee_zcl_thermostat_sws_day_of_week[] = {
    { 0x01, "Sunday" },
    { 0x02, "Monday" },
    { 0x04, "Tuesday" },
    { 0x08, "Wednesday" },
...
static void dissect_zbee_zcl(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
    ...
    if ( packet.frame_type == ZBEE_ZCL_FCF_PROFILE_WIDE ) {
        ...
        if ( pinfo->zbee_cluster_id == ZBEE_ZCL_CID_THERMOSTAT ) {
            if ( check_col(pinfo->cinfo, COL_INFO) ) {
                col_append_fstr(pinfo->cinfo, COL_INFO, "Command: %s, Seq: %u", val_to_str(packet.cmd_id,
                    zbee_zcl_thermostat_cmd_names, "unknown Thermostat command"), packet.tran_seqno);
            }
            if ( zcl_tree ) {
                if ( packet.direction == ZBEE_ZCL_DIR_REPORTED /* We have a client */ ) {
                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_thermostat_cmd_id, tvb, offset, (int)sizeof(guint8),
                        packet.cmd_id);
                    switch ( packet.cmd_id ) {
                        case ZBEE_ZCL_CSC_THERMOSTAT_C_SRL: /* Setpoint Raise/Lower */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_thermostat_setpointrl_modes, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            proto_tree_add_text(zcl_tree, tvb, offset + 2 * (int)sizeof(guint8), (int)sizeof(guint8),
                                "Amount Field: increased/decreased by %d, in steps of 0.1 C", *(tvb->real_data + offset + 2 *
                                (int)sizeof(guint8)));
                            break;
                        case ZBEE_ZCL_CSC_THERMOSTAT_C_SWS: /* Set Weekly Schedule */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_thermostat_sws_n_trans, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            ti = proto_tree_add_text(zcl_tree, tvb, offset + 2 * (int)sizeof(guint8), (int)sizeof(guint8),
                                "Day of Week for Sequence: 0x%02x", tvb_get_guint8(tvb, offset + 2 * (int)sizeof(guint8)));
                            sub_tree = proto_item_add_subtree(ti, ett_zbee_zcl_sws_dow_for_sequence);
                            for (i = 0 ; i < 8 ; ++i ) {
                                if ( tvb_get_guint8(tvb, (offset + 2 * (int)sizeof(guint8))) & (0x01 << i) ) {
                                    proto_tree_add_uint(sub_tree, hf_zbee_zcl_thermostat_sws_day_of_week, tvb, offset + 2 *
                                        (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, (offset + 2 *
                                        (int)sizeof(guint8))) & (0x01 << i));
                                }
                            }
                            ti = proto_tree_add_text(zcl_tree, tvb, offset + 3 * (int)sizeof(guint8), (int)sizeof(guint8),
                                "Mode for Sequence");
                            sub_tree = proto_item_add_subtree(ti, ett_zbee_zcl_sws_mfs);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_thermostat_sws_mfs_heat, tvb, offset + 3 *
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + 3 *
                                (int)sizeof(guint8)));
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_thermostat_sws_mfs_cool, tvb, offset + 3 *
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + 3 *
                                (int)sizeof(guint8)));
                            switch ( tvb_get_guint8(tvb, offset + 3 * (int)sizeof(guint8)) ) {
                                case ZBEE_ZCL_CSC_THERMOSTAT_C_SWS_HSP: /* Heat Set Point */
                                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_thermostat_sws_trans_time_1, tvb, offset + 4 *
                                        (int)sizeof(guint8), (int)sizeof(guint16), tvb_get_letohs(tvb, offset + 4 *
                                        (int)sizeof(guint8)));
                                    proto_tree_add_int(zcl_tree, hf_zbee_zcl_thermostat_sws_hsp_1, tvb, offset + 6 *
                                        (int)sizeof(guint8), (int)sizeof(guint16), tvb_get_letohs(tvb, offset + 6 *
                                        (int)sizeof(guint8)));
                                    break;
                                case ZBEE_ZCL_CSC_THERMOSTAT_C_SWS_CSP: /* Cool Set Point */
                                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_thermostat_sws_trans_time_1, tvb, offset + 4 *
                                        (int)sizeof(guint8), (int)sizeof(guint16), tvb_get_letohs(tvb, offset + 4 *
                                        (int)sizeof(guint8)));
                                    proto_tree_add_int(zcl_tree, hf_zbee_zcl_thermostat_sws_csp_1, tvb, offset + 6 *
                                        (int)sizeof(guint8), (int)sizeof(guint16), tvb_get_letohs(tvb, offset + 6 *
                                        (int)sizeof(guint8)));
                                    break;
                                case ZBEE_ZCL_CSC_THERMOSTAT_C_SWS_B: /* Both */
                                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_thermostat_sws_trans_time_1, tvb, offset + 4 *
                                        (int)sizeof(guint8), (int)sizeof(guint16), tvb_get_letohs(tvb, offset + 4 *
                                        (int)sizeof(guint8)));
                                    proto_tree_add_int(zcl_tree, hf_zbee_zcl_thermostat_sws_hsp_1, tvb, offset + 6 *
                                        (int)sizeof(guint8), (int)sizeof(guint16), tvb_get_letohs(tvb, offset + 6 *
                                        (int)sizeof(guint8)));
                                    proto_tree_add_int(zcl_tree, hf_zbee_zcl_thermostat_sws_csp_1, tvb, offset + 8 *
                                        (int)sizeof(guint8), (int)sizeof(guint16), tvb_get_letohs(tvb, offset + 8 *
                                        (int)sizeof(guint8)));
                                    break;
                            }
                            break;
                        case ZBEE_ZCL_CSC_THERMOSTAT_C_GWS: /* Get Weekly Schedule */
                            ti = proto_tree_add_text(zcl_tree, tvb, offset + (int)sizeof(guint8), (int)sizeof(guint8),
                                "Days To Return: 0x%02x", tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            sub_tree = proto_item_add_subtree(ti, ett_zbee_zcl_gws_days_to_return);
                            for (i = 0 ; i < 8 ; ++i ) {
                                if ( tvb_get_guint8(tvb, (offset + (int)sizeof(guint8))) & (0x01 << i) ) {
                                    proto_tree_add_uint(sub_tree, hf_zbee_zcl_thermostat_gws_days_to_return, tvb, offset + 
                                        (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, (offset +
                                        (int)sizeof(guint8))) & (0x01 << i));
                                }
                            }
                            ti = proto_tree_add_text(zcl_tree, tvb, offset + 2 * (int)sizeof(guint8), (int)sizeof(guint8),
                                "Mode To Return");
                            sub_tree = proto_item_add_subtree(ti, ett_zbee_zcl_gws_mtr);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_thermostat_gws_mtr_heat, tvb, offset + 2 *
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + 2 *
                                (int)sizeof(guint8)));
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_thermostat_gws_mtr_cool, tvb, offset + 2 *
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + 2 *
                                (int)sizeof(guint8)));
                            break;
                    }
        ...
        } else if ( pinfo->zbee_cluster_id == ZBEE_ZCL_CID_IAS_ZONE ) {
            if ( packet.direction == ZBEE_ZCL_DIR_REPORTED /* We have a client */ ) {
                if ( check_col(pinfo->cinfo, COL_INFO) ) {
                    col_append_fstr(pinfo->cinfo, COL_INFO, "Command: %s, Seq: %u", val_to_str(packet.cmd_id,
                ...
                if ( zcl_tree ) {
                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ias_zone_client_cmd_id, tvb, offset, (int)sizeof(guint8),
                        packet.cmd_id);
                    switch ( packet.cmd_id ) {
                        case ZBEE_ZCL_CSC_IAS_ZONE_C_ZER: /* Zone Enroll Response */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ias_zone_client_erc, tvb, offset + (int)sizeof(guint8),
                                (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ias_zone_client_zone_id, tvb, offset + 2 *
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + 2 *
                                (int)sizeof(guint8)));
                            break;
                    }
                ...
                if ( check_col(pinfo->cinfo, COL_INFO) ) {
                    col_append_fstr(pinfo->cinfo, COL_INFO, "Command: %s, Seq: %u", val_to_str(packet.cmd_id,
                        zbee_zcl_ias_zone_cmd_names, "unknown IAS Zone server command"), packet.tran_seqno);
                }
                if ( zcl_tree ) {
                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ias_zone_cmd_id, tvb, offset, (int)sizeof(guint8),
                        packet.cmd_id);
                    switch ( packet.cmd_id ) {
                        case ZBEE_ZCL_CSC_IAS_ZONE_S_ZSCN: /* Zone Status Change Notification */
                            ti = proto_tree_add_text(zcl_tree, tvb, offset + (int)sizeof(guint8), 2 * (int)sizeof(guint8),
                                "Zone Status: 0x%02x", tvb_get_letohs(tvb, offset + (int)sizeof(guint8)));
                            sub_tree = proto_item_add_subtree(ti, ett_zbee_zcl_scn_zone_status);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ias_zone_scn_alarm1, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)) & 0x01);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ias_zone_scn_alarm2, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)) & 0x02);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ias_zone_scn_tamper, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)) & 0x04);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ias_zone_scn_battery, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)) & 0x08);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ias_zone_scn_supervision_reports, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)) & 0x10);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ias_zone_scn_restore_reports, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)) & 0x20);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ias_zone_scn_trouble, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)) & 0x40);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ias_zone_scn_ac_mains, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)) & 0x80);
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ias_zone_scn_ext_status, tvb, offset + 3 *
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + 3 *
                                (int)sizeof(guint8)));
                            break;
                    }
        ...
        } else if ( pinfo->zbee_cluster_id == ZBEE_ZCL_CID_POLL_CONTROL ) {
            if ( packet.direction == ZBEE_ZCL_DIR_REPORTED /* We have a client */ ) {
                if ( check_col(pinfo->cinfo, COL_INFO) ) {
                    col_append_fstr(pinfo->cinfo, COL_INFO, "Command: %s, Seq: %u", val_to_str(packet.cmd_id,
                        zbee_zcl_poll_control_client_cmd_names, "unknown Poll Control client command"), packet.tran_seqno);
                }
                if ( zcl_tree ) {
                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_poll_control_client_cmd_id, tvb, offset, (int)sizeof(guint8),
                        packet.cmd_id);
                    switch ( packet.cmd_id ) {
                        case ZBEE_ZCL_CSC_POLL_CONTROL_C_CIR: /* Check-in Response */
                            proto_tree_add_boolean(zcl_tree, hf_zbee_zcl_poll_control_client_sfp, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_poll_control_client_fpt, tvb, offset + 2 *
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 2 *
                                (int)sizeof(guint8)));
                            break;
                        case ZBEE_ZCL_CSC_POLL_CONTROL_C_SLPI: /* Set Long Poll Interval */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_poll_control_client_nlpi, tvb, offset +
                                (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset +
                                (int)sizeof(guint8)));
                            break;
                        case ZBEE_ZCL_CSC_POLL_CONTROL_C_SSPI: /* Set Short Poll Interval */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_poll_control_client_nspi, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)));
                            break;
                      }
                ...
                if ( check_col(pinfo->cinfo, COL_INFO) ) {
                    col_append_fstr(pinfo->cinfo, COL_INFO, "Command: %s, Seq: %u", val_to_str(packet.cmd_id,
                        zbee_zcl_poll_control_server_cmd_names, "unknown Poll Control server command"), packet.tran_seqno);
                }
                if ( zcl_tree ) {
                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_poll_control_server_cmd_id, tvb, offset, (int)sizeof(guint8),
                        packet.cmd_id);
                }
                offset = tvb_length(tvb);
            }
        } else if ( pinfo->zbee_cluster_id == ZBEE_ZCL_CID_IDENTIFY ) {
            if ( packet.direction == ZBEE_ZCL_DIR_REPORTED /* We have a client */ ) {
                if ( check_col(pinfo->cinfo, COL_INFO) ) {
                    col_append_fstr(pinfo->cinfo, COL_INFO, "Command: %s, Seq: %u", val_to_str(packet.cmd_id,
                ...
                if ( zcl_tree ) {
                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_identify_client_cmd_id, tvb, offset, (int)sizeof(guint8),
                        packet.cmd_id);
                    switch ( packet.cmd_id ) {
                        case ZBEE_ZCL_CSC_IDENTIFY_C_I: /* Identify */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_identify_client_identify_time, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)));
                            break;
                    }
                ...
                if ( zcl_tree ) {
                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_identify_server_cmd_id, tvb, offset, (int)sizeof(guint8),
                        packet.cmd_id);
                    switch ( packet.cmd_id ) {
                        case ZBEE_ZCL_CSC_IDENTIFY_S_IQR: /* Identify Query Response */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_identify_server_timeout, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)));
                            break;
                    }
        ...
        } else if ( pinfo->zbee_cluster_id == ZBEE_ZCL_CID_OTA_UPGRADE ) {
            if ( packet.direction == ZBEE_ZCL_DIR_REPORTED /* We have a client */ ) {
                if ( check_col(pinfo->cinfo, COL_INFO) ) {
                    col_append_fstr(pinfo->cinfo, COL_INFO, "Command: %s, Seq: %u", val_to_str(packet.cmd_id,
                        zbee_zcl_ota_upgrade_client_cmd_names, "unknown OTA Upgrade client command"), packet.tran_seqno);
                }
                if ( zcl_tree ) {
                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_cmd_id, tvb, offset, (int)sizeof(guint8),
                        packet.cmd_id);
                    switch ( packet.cmd_id ) {
                        case ZBEE_ZCL_CSC_OTA_UPGRADE_C_QNIR: /* Query Next Image Request */
                            ti = proto_tree_add_text(zcl_tree, tvb, offset + (int)sizeof(guint8), (int)sizeof(guint8),
                                "Field Control");
                            sub_tree = proto_item_add_subtree(ti, ett_zbee_zcl_qnir_field_control);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ota_upgrade_client_qnir_hvp, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_qnir_manuf_code, tvb, offset + 2 *
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 2 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_qnir_image_type, tvb, offset + 4 *
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 4 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_qnir_cfv, tvb, offset + 6 *
                                (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 6 *
                                (int)sizeof(guint8)));
                            if (tvb_get_guint8(tvb, offset + (int)sizeof(guint8))) {
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_qnir_hv, tvb, offset + 10 *
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 10 *
                                (int)sizeof(guint8)));
                            }
                            break;
                        case ZBEE_ZCL_CSC_OTA_UPGRADE_C_IBR: /* Image Block Request */
                            ti = proto_tree_add_text(zcl_tree, tvb, offset + (int)sizeof(guint8), (int)sizeof(guint8),
                                "Field Control");
                            sub_tree = proto_item_add_subtree(ti, ett_zbee_zcl_ibr_field_control);
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ota_upgrade_client_ibr_ieee, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            proto_tree_add_boolean(sub_tree, hf_zbee_zcl_ota_upgrade_client_ibr_brdp, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_ibr_mc, tvb, offset + 2 *
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 2 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_ibr_it, tvb, offset + 4 *
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 4 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_ibr_fv, tvb, offset + 6 *
                                (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 6 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_ibr_fo, tvb, offset + 10 *
                                (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 10 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_ibr_mds, tvb, offset + 14 *
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + 14 *
                                (int)sizeof(guint8)));
                            temporary_offset = offset + 15 * (int)sizeof(guint8);
                            if (tvb_get_guint8(tvb, offset + (int)sizeof(guint8)) & 0x01) {
                                proto_tree_add_item(zcl_tree, hf_zbee_zcl_ota_upgrade_client_ibr_ieee_address, tvb,
                                    temporary_offset, 8 * (int)sizeof(guint8), ENC_LITTLE_ENDIAN);
                                temporary_offset += 8 * (int)sizeof(guint8);
                            }
                            if (tvb_get_guint8(tvb, offset + (int)sizeof(guint8)) & 0x02) {
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_ibr_brd, tvb, temporary_offset, 2
                                    * (int)sizeof(guint8), tvb_get_letohs(tvb, temporary_offset));
                            }
                            break;
                        case ZBEE_ZCL_CSC_OTA_UPGRADE_C_UER: /* Upgrade End Request */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_uer_status, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_uer_mc, tvb, offset + 2 *
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 2 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_uer_it, tvb, offset + 4 *
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 4 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_client_uer_fv, tvb, offset + 6 *
                                (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 6 *
                                (int)sizeof(guint8)));
                            break;
                    }
                ...
                if ( check_col(pinfo->cinfo, COL_INFO) ) {
                    col_append_fstr(pinfo->cinfo, COL_INFO, "Command: %s, Seq: %u", val_to_str(packet.cmd_id,
                        zbee_zcl_ota_upgrade_server_cmd_names, "unknown OTA Upgrade server command"), packet.tran_seqno);
                }
                if ( zcl_tree ) {
                    proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_cmd_id, tvb, offset, (int)sizeof(guint8),
                        packet.cmd_id);
                    switch ( packet.cmd_id ) {
                        case ZBEE_ZCL_CSC_OTA_UPGRADE_S_IN: /* Image Notify */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_in_pt, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_in_qj, tvb, offset + 2 *
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + 2 *
                                (int)sizeof(guint8)));
                            if (tvb_get_guint8(tvb, offset + (int)sizeof(guint8)) > 0x00) {
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_in_mc, tvb, offset + 3 *
                                    (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 3 *
                                    (int)sizeof(guint8)));
                            }
                            if (tvb_get_guint8(tvb, offset + (int)sizeof(guint8)) > 0x01) {
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_in_it, tvb, offset + 5 *
                                    (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 5 *
                                    (int)sizeof(guint8)));
                            }
                            if (tvb_get_guint8(tvb, offset + (int)sizeof(guint8)) > 0x02) {
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_in_nfv, tvb, offset + 7 *
                                    (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 7 *
                                    (int)sizeof(guint8)));
                            }
                            break;
                        case ZBEE_ZCL_CSC_OTA_UPGRADE_S_QNIR: /* Query Next Image Response */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_qnir_status, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            if (!tvb_get_guint8(tvb, offset + (int)sizeof(guint8))) {
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_qnir_mc, tvb, offset + 2 *
                                    (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 2 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_qnir_it, tvb, offset + 4 *
                                    (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 4 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_qnir_fv, tvb, offset + 6 *
                                    (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 6 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_qnir_is, tvb, offset + 10 *
                                    (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 10 *
                                    (int)sizeof(guint8)));
                            }
                            break;
                        case ZBEE_ZCL_CSC_OTA_UPGRADE_S_IBR: /* Image Block Response */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_status, tvb, offset +
                                (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + (int)sizeof(guint8)));
                            if (tvb_get_guint8(tvb, offset + (int)sizeof(guint8)) == ZBEE_ZCL_CSC_OTA_UPGRADE_S_IBR_WFD)
                                /* Wait For Data */ {
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_wfd_ct, tvb, offset + 2 *
                                    (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 2 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_wfd_rt, tvb, offset + 6 *
                                    (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 6 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_wfd_brd, tvb, offset + 10 *
                                    (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 10 *
                                    (int)sizeof(guint8)));
                            } else if (tvb_get_guint8(tvb, offset + (int)sizeof(guint8)) == ZBEE_ZCL_CSC_OTA_UPGRADE_S_IBR_S)
                                /* Success */ {
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_mc, tvb, offset + 2 *
                                    (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 2 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_it, tvb, offset + 4 *
                                    (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 4 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_fv, tvb, offset + 6 *
                                    (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 6 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_fo, tvb, offset + 10 *
                                    (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 10 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_ds, tvb, offset + 14 *
                                    (int)sizeof(guint8), (int)sizeof(guint8), tvb_get_guint8(tvb, offset + 14 *
                                    (int)sizeof(guint8)));
                                proto_tree_add_item(zcl_tree, hf_zbee_zcl_ota_upgrade_server_ibr_image_data, tvb, offset + 15,
                                    tvb_length_remaining(tvb, offset + 15), ENC_LITTLE_ENDIAN);
                            }
                            break;
                        case ZBEE_ZCL_CSC_OTA_UPGRADE_S_UER: /* Upgrade End Response */
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_uer_mc, tvb, offset +
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset +
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_uer_it, tvb, offset + 3 *
                                (int)sizeof(guint8), 2 * (int)sizeof(guint8), tvb_get_letohs(tvb, offset + 3 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_uer_fv, tvb, offset + 5 *
                                (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 5 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_uer_ct, tvb, offset + 9 *
                                (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 9 *
                                (int)sizeof(guint8)));
                            proto_tree_add_uint(zcl_tree, hf_zbee_zcl_ota_upgrade_server_uer_ut, tvb, offset + 13 *
                                (int)sizeof(guint8), 4 * (int)sizeof(guint8), tvb_get_letohl(tvb, offset + 13 *
                                (int)sizeof(guint8)));
                            break;
                    }
                }
            ...
            if ( check_col(pinfo->cinfo, COL_INFO) ) {
                col_append_fstr(pinfo->cinfo, COL_INFO, "Command: 0x%02x, Seq: %u", packet.cmd_id, packet.tran_seqno);
            }
            if ( zcl_tree ) {
                proto_tree_add_uint(zcl_tree, hf_zbee_zcl_cs_cmd_id, tvb, offset, (int)sizeof(guint8), packet.cmd_id);
            }
            offset += (int)sizeof(guint8);
        }
...
void proto_register_zbee_zcl(void)
{
    ...
    static const true_false_string tfs_supervision_reports = {
        "Reports",
        "Does not report"
    };

    static const true_false_string tfs_restore_reports = {
        "Reports restore",
    ...
    static hf_register_info hf[] = {
        ...
        { &hf_zbee_zcl_thermostat_cmd_id,
            { "Command",    "zbee_zcl.thermostat.cmd.id", FT_UINT8, BASE_HEX, VALS(zbee_zcl_thermostat_cmd_names),
                0x0, NULL, HFILL }},

        { &hf_zbee_zcl_ias_zone_cmd_id,
            { "Command", "zbee_zcl.ias_zone.cmd.id", FT_UINT8, BASE_HEX, VALS(zbee_zcl_ias_zone_cmd_names), 0x0, NULL,
                HFILL }},
        ...
        { &hf_zbee_zcl_ias_zone_client_cmd_id,
            { "Command", "zbee_zcl.ias_zone.client.cmd.id", FT_UINT8, BASE_HEX, VALS(zbee_zcl_ias_zone_client_cmd_names), 0x0,
                NULL, HFILL }},

        { &hf_zbee_zcl_ias_zone_client_erc,
            { "Enroll response code", "zbee_zcl.ias_zone.client.erc", FT_UINT8, BASE_HEX, VALS(zbee_zcl_ias_zone_client_erc),
                0x0, NULL, HFILL }},

        { &hf_zbee_zcl_ias_zone_client_zone_id,
            { "Zone ID", "zbee_zcl.ias_zone.client.zone_id", FT_UINT8, BASE_HEX, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_poll_control_client_cmd_id,
            { "Command", "zbee_zcl.poll_control.client.cmd.id", FT_UINT8, BASE_HEX,
        ...
        { &hf_zbee_zcl_ota_upgrade_server_ibr_image_data,
            { "Image Data", "zbee_zcl.ota_upgrade.server.ibr.image_data", FT_BYTES, BASE_NONE, NULL, 0x0, NULL, HFILL }},
        ...
        { &hf_zbee_zcl_ota_upgrade_client_ibr_ieee_address,
            { "IEEE Address", "zbee_zcl.ota_upgrade.client.ibr.ieee_address", FT_EUI64, BASE_NONE, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_ibr_fo,
            { "File Offset (in bytes)", "zbee_zcl.ota_upgrade.client_ibr_fo", FT_UINT32, BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_ibr_mds,
            { "Maximum Data Size (in bytes)", "zbee_zcl.ota_upgrade.client_ibr_mds", FT_UINT8, BASE_DEC, NULL, 0x0, NULL,
                HFILL }},
        ...
        { &hf_zbee_zcl_ota_upgrade_server_qnir_mc,
            { "Manufacturer Code", "zbee_zcl.ota_upgrade.server.qnir.mc", FT_UINT16, BASE_HEX, NULL, 0x0, NULL, HFILL }},
        ...
        { &hf_zbee_zcl_ota_upgrade_server_qnir_is,
            { "Image Size (in bytes)", "zbee_zcl.ota_upgrade.server.qnir.is", FT_UINT32, BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_identify_client_cmd_id,
            { "Command", "zbee_zcl.identify.client.cmd.id", FT_UINT8, BASE_HEX, VALS(zbee_zcl_identify_client_cmd_names), 0x0,
                NULL, HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_cmd_id,
            { "Command", "zbee_zcl.ota_upgrade.client.cmd.id", FT_UINT8, BASE_HEX, VALS(zbee_zcl_ota_upgrade_client_cmd_names),
                0x0, NULL, HFILL }},

        { &hf_zbee_zcl_ota_upgrade_server_cmd_id,
            { "Command", "zbee_zcl.ota_upgrade.server.cmd.id", FT_UINT8, BASE_HEX, VALS(zbee_zcl_ota_upgrade_server_cmd_names),
                0x0, NULL, HFILL }},

        { &hf_zbee_zcl_identify_server_cmd_id,
            { "Command", "zbee_zcl.identify.server.cmd.id", FT_UINT8, BASE_HEX, VALS(zbee_zcl_identify_server_cmd_names), 0x0,
                NULL, HFILL }},

        { &hf_zbee_zcl_poll_control_server_cmd_id,
            { "Command", "zbee_zcl.poll_control.server.cmd.id", FT_UINT8, BASE_HEX,
                VALS(zbee_zcl_poll_control_server_cmd_names), 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_poll_control_client_sfp,
            { "Start Fast Polling", "zbee_zcl.poll_control.client.sfp", FT_BOOLEAN, 8, TFS(&tfs_true_false), 0x00, NULL,
                HFILL }},

        { &hf_zbee_zcl_poll_control_client_fpt,
            { "Fast Poll Timeout (quarterseconds)", "zbee_zcl.poll_control.client.fpt", FT_UINT16, BASE_DEC, NULL, 0x0, NULL,
                HFILL }},

        { &hf_zbee_zcl_poll_control_client_nlpi,
            { "New Long Poll Interval", "zbee_zcl.poll_control.client.nlpi", FT_UINT32, BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_poll_control_client_nspi,
            { "New Short Poll Interval", "zbee_zcl.poll_control.client.nspi", FT_UINT16, BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_identify_client_identify_time,
            { "Identify Time (seconds)", "zbee_zcl.identify.client.identify_time", FT_UINT16, BASE_DEC, NULL, 0x0, NULL,
                HFILL }},

        { &hf_zbee_zcl_identify_server_timeout,
            { "Timeout (seconds)", "zbee_zcl.identify.server.timeout", FT_UINT16, BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_qnir_manuf_code,
            { "Manufacturer Code", "zbee_zcl.ota_upgrade.client.qnir.manufacturer_code", FT_UINT16, BASE_HEX, NULL, 0x0, NULL,
                HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_qnir_hv,
            { "Hardware Version", "zbee_zcl.ota_upgrade.client.qnir.hardware_version", FT_UINT16, BASE_HEX, NULL, 0x0, NULL,
                HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_qnir_image_type,
            { "Image Type", "zbee_zcl.ota_upgrade.client.qnir.image_type", FT_UINT16, BASE_HEX, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_qnir_cfv,
            { "Current File Version", "zbee_zcl.ota_upgrade.client.qnir.cfv", FT_UINT32, BASE_HEX, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_ias_zone_scn_ext_status,
            { "Extended Status", "zbee_zcl.ias_zone.scn.ext_status", FT_UINT8, BASE_HEX, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_setpointrl_modes,
            { "Mode Field", "zbee_zcl.thermostat.setpoint_raise_lower.mode_field", FT_UINT8, BASE_HEX,
                VALS(zbee_zcl_thermostat_setpointrl_modes), 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_sws_n_trans,
            { "Number of Transitions for Sequence", "zbee_zcl.thermostat.sws.ntrans", FT_UINT8, BASE_HEX, NULL, 0x0, NULL,
                HFILL }},
        
        { &hf_zbee_zcl_thermostat_sws_day_of_week,
            { "Day of Week for Sequence", "zbee_zcl.thermostat.set_weekly_schedule.day_of_week_for_schedule", FT_UINT8,
                BASE_HEX, VALS(zbee_zcl_thermostat_sws_day_of_week), 0x00, NULL, HFILL }},
        
        { &hf_zbee_zcl_thermostat_gws_days_to_return,
            { "Days To Return", "zbee_zcl.thermostat.get_weekly_schedule.days_to_return", FT_UINT8, BASE_HEX,
                VALS(zbee_zcl_thermostat_sws_day_of_week), 0x00, NULL, HFILL }},
        
        { &hf_zbee_zcl_ias_zone_scn_alarm1,
            { "Alarm 1", "zbee_zcl.ias_zone.scn.alarm_1", FT_BOOLEAN, 16, TFS(&tfs_alarmed_or_not), 0x01, NULL, HFILL }},
        
        { &hf_zbee_zcl_ias_zone_scn_alarm2,
            { "Alarm 2", "zbee_zcl.ias_zone.scn.alarm_2", FT_BOOLEAN, 16, TFS(&tfs_alarmed_or_not), 0x02, NULL, HFILL }},
        
        { &hf_zbee_zcl_ias_zone_scn_tamper,
            { "Tamper", "zbee_zcl.ias_zone.scn.tamper", FT_BOOLEAN, 16, TFS(&tfs_tampered_or_not), 0x04, NULL, HFILL }},
        
        { &hf_zbee_zcl_ias_zone_scn_battery,
            { "Battery", "zbee_zcl.ias_zone.scn.battery", FT_BOOLEAN, 16, TFS(&tfs_battery), 0x08, NULL, HFILL }},
        
        { &hf_zbee_zcl_ias_zone_scn_supervision_reports,
            { "Supervision Reports", "zbee_zcl.ias_zone.scn.supervision_reports", FT_BOOLEAN, 16,
                TFS(&tfs_supervision_reports), 0x10, NULL, HFILL }},
        
        { &hf_zbee_zcl_ias_zone_scn_restore_reports,
            { "Restore Reports", "zbee_zcl.ias_zone.scn.restore_reports", FT_BOOLEAN, 16, TFS(&tfs_restore_reports), 0x20,
                NULL, HFILL }},
        
        { &hf_zbee_zcl_ias_zone_scn_trouble,
            { "Trouble", "zbee_zcl.ias_zone.scn.trouble", FT_BOOLEAN, 16, TFS(&tfs_trouble_failure), 0x40, NULL, HFILL }},
        
        { &hf_zbee_zcl_ias_zone_scn_ac_mains,
            { "AC (mains)", "zbee_zcl.ias_zone.scn.ac_mains", FT_BOOLEAN, 16, TFS(&tfs_ac_mains), 0x80, NULL, HFILL }},
        
        { &hf_zbee_zcl_thermostat_sws_mfs_heat,
            { "Heat Setpoint Field Present in Payload", "zbee_zcl.thermostat.set_weekly_schedule.mfs_heat", FT_BOOLEAN, 8,
                NULL, 0x01, NULL, HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_qnir_hvp,
            { "Hardware Version Present", "zbee_zcl.ota_upgrade.client.qnir.field_control", FT_BOOLEAN, 8, NULL, 0x01, NULL,
                HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_ibr_ieee,
            { "Request Node's IEEE Address Presents", "zbee_zcl.ota_upgrade.client.ibr.ieee", FT_BOOLEAN, 8, NULL, 0x01, NULL,
                HFILL }},
        ...
        { &hf_zbee_zcl_thermostat_sws_mfs_cool,
            { "Cool Setpoint Field Present in Payload", "zbee_zcl.thermostat.set_weekly_schedule.mfs_cool", FT_BOOLEAN, 8,
                NULL, 0x02, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_gws_mtr_heat,
            { "Heat Setpoint Field Present in Payload", "zbee_zcl.thermostat.get_weekly_schedule.mtr_heat", FT_BOOLEAN, 8,
                  NULL, 0x01, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_gws_mtr_cool,
            { "Cool Setpoint Field Present in Payload", "zbee_zcl.thermostat.get_weekly_schedule.mtr_cool", FT_BOOLEAN, 8,
                  NULL, 0x02, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_sws_trans_time_1,
            { "Transition Time 1 (minutes since midnight)", "zbee_zcl.thermostat.set_weekly_schedule.trans_time_1",
                  FT_UINT16, BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_sws_trans_time_2,
            { "Transition Time 2 (minutes since midnight)", "zbee_zcl.thermostat.set_weekly_schedule.trans_time_2",
                  FT_UINT16, BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_sws_hsp_1,
            { "Heat Set Point 1 (with 0.01 C resolution)", "zbee_zcl.thermostat.set_weekly_schedule.hsp_1", FT_INT16,
                  BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_sws_hsp_2,
            { "Heat Set Point 2 (with 0.01 C resolution)", "zbee_zcl.thermostat.set_weekly_schedule.hsp_2", FT_INT16,
                  BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_sws_csp_1,
            { "Cool Set Point 1 (with 0.01 C resolution)", "zbee_zcl.thermostat.set_weekly_schedule.csp_1", FT_INT16,
                  BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_thermostat_sws_csp_2,
            { "Cool Set Point 2 (with 0.01 C resolution)", "zbee_zcl.thermostat.set_weekly_schedule.csp_2", FT_INT16,
                  BASE_DEC, NULL, 0x0, NULL, HFILL }},

        { &hf_zbee_zcl_ota_upgrade_client_ibr_brd,
            { "Block Request Delay", "zbee_zcl.ota_upgrade.client.ibr.brd", FT_UINT16, BASE_DEC, NULL, 0x0, NULL, HFILL }},
    ...
    ett[2] = &ett_zbee_zcl_sws_dow_for_sequence; 
    ett[3] = &ett_zbee_zcl_sws_mfs; 
    ett[4] = &ett_zbee_zcl_gws_days_to_return; 
    ett[5] = &ett_zbee_zcl_gws_mtr; 
    ett[6] = &ett_zbee_zcl_scn_zone_status; 
    ett[7] = &ett_zbee_zcl_qnir_field_control;
    ett[8] = &ett_zbee_zcl_ibr_field_control;
...
/* Dissector functions for Green Power profiles 
 * needed on the NWK-GP layer (out of bounds ZCL layer) 
 */

/*FUNCTION:------------------------------------------------------
 *  NAME
 *      dissect_zcl_attributes_payload
 *  DESCRIPTION
 *      Helper dissector for parse ZCL attribute request command 
 *      can be used outside of the ZCL Level
 *  PARAMETERS
 *      tvbuff_t *tvb       - pointer to buffer containing raw packet.
 *      packet_info *pinfo  - pointer to packet information fields
 *      proto_tree *tree    - pointer to data tree wireshark uses to display packet.
