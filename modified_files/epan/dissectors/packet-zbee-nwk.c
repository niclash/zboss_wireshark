...
static void        dissect_zbee_nwk_full   (tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree);
 ...
 *      dissect_zbee_nwk
 *  DESCRIPTION
 *      ZigBee NWK packet Wireshark dissection with proto version determination.
 *  PARAMETERS
 *      tvbuff_t *tvb       - pointer to buffer containing raw packet.
 *      packet_into *pinfo  - pointer to packet information fields
 *      proto_tree *tree    - pointer to data tree Wireshark uses to display packet.
...
dissect_zbee_nwk(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
  ...
  if (proto_version == ZBEE_VERSION_GREEN_POWER) {
    //See: ZigBee Green Power Profile
    dissector_handle_t zbee_gp_handle = find_dissector(ZBEE_PROTOABBREV_NWK_GP);
    call_dissector(zbee_gp_handle, tvb, pinfo, tree);
  } else {
    //TODO: Add check for fcf proto versions
    dissect_zbee_nwk_full(tvb, pinfo, tree);
  }
}
/*FUNCTION:------------------------------------------------------
 *  NAME
 ...
 *  DESCRIPTION
 *      ZigBee NWK packet Wireshark dissection routine for 2006, 2007 and Pro stack versions.
 *  PARAMETERS
 *      tvbuff_t *tvb       - pointer to buffer containing raw packet.
 *      packet_into *pinfo  - pointer to packet information fields
 *      proto_tree *tree    - pointer to data tree Wireshark uses to display packet.
 ...
 */
static void
dissect_zbee_nwk_full(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
